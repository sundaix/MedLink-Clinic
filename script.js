...
function validatePhoneNumber(phone) {
    ...
    return /^09[0-9]{9}$/.test(cleanPhone);
}
...
document.getElementById('appointmentForm').addEventListener('submit', async function(e) {
    ...
    if (phone.length !== 11) {
        showNotification('Phone number must be exactly 11 digits (09XX XXX XXXX)', 'error');
        return;
    }
    
    if (!validatePhoneNumber(phone)) {
        showNotification('Please enter a valid Philippine mobile number starting with 09', 'error');
        return;
    }
...
// Smart phone number formatting with spaces
function setupPhoneNumberFormatting() {
    ...
    // Allow proper cursor positioning and deletion
    phoneInput.addEventListener('keydown', function(e) {
        // Store current cursor position
        const cursorPosition = e.target.selectionStart;
        const valueBefore = e.target.value;
        
        // If backspace/delete at space position, jump over space
        if ((e.key === 'Backspace' || e.key === 'Delete') && cursorPosition > 0) {
            const charBefore = valueBefore.charAt(cursorPosition - 1);
            const charAt = valueBefore.charAt(cursorPosition);
            
            // If backspace before a space, delete the digit before space
            if (e.key === 'Backspace' && charBefore === ' ') {
                e.preventDefault();
                const newValue = valueBefore.substring(0, cursorPosition - 2) + valueBefore.substring(cursorPosition);
                e.target.value = newValue;
                e.target.setSelectionRange(cursorPosition - 2, cursorPosition - 2);
                return;
            }
            
            // If delete on a space, jump over it
            if (e.key === 'Delete' && charAt === ' ') {
                e.preventDefault();
        ...
        if ([8, 9, 13, 27, 37, 38, 39, 40, 46, 36, 35].includes(e.keyCode)) {
            return;
        }
